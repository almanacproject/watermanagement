...
SharedAccessSignature.prototype.signRequest = function (webResource, callback) {
  ...
  var expirationDate = 1461804309;//Math.round(date.minutesFromNow(5) / 1000);
  ...
  webResource.withHeader(HeaderConstants.AUTHORIZATION,
    ...
    console.log(util.format('SharedAccessSignature sig=%s&se=%s&skn=%s&sr=%s', signature, expirationDate, this.keyName, targetUri));
...
SharedAccessSignature.prototype._generateSignature = function (targetUri, expirationDate) {
  ...
  var stringToSign = getvalueToAppend(targetUri) +
    ...
    console.log('magic stuff:\t' + encodeURIComponent(crypto.createHmac('sha256', this.keyValue).update(stringToSign).digest('base64')));
